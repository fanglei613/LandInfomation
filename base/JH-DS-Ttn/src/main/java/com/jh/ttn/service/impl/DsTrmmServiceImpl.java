...
import com.jh.ttn.mapping.IDsTrmmMapper;
...
public class DsTrmmServiceImpl implements IDsTrmmService {
    ...
    @Autowired
    private IDsTrmmMapper trmmMapper;
     ...
     */
    @Override
    public ResultMessage findTrmmForThree(Long regionId, String startDate, String endDate,Integer resolution ) {
        //检验参数，校验通过方法才可继续
        ...
        if(result.isFlag())
        {
            ...
            for(int i=0;i<avgYearNum;i++){
                ...
                List<TtnVo> dataList = trmmMapper.findTrmmByRegion(param);
    ...
    */
    @Override
    public ResultMessage findTrmmForMon(Long regionId, String startDate, String endDate,Integer resolution ) {
        //检验参数，校验通过方法才可继续
        ResultMessage result = paramCheck(regionId,startDate,endDate,resolution);
        if(result.isFlag()){
            LocalDate sDate = DateUtil.strToLocalDate(startDate); //开始如期
            ...
            param.setResolution(resolution);
            //获取查询日期的数据
            List<TtnVo> curList = trmmMapper.findTrmm(param);
            ...
            List<TtnVo> lastList = trmmMapper.findTrmm(param);

            //将前一月的数据放入lastValue字段中，并计算percent.
    ...
    public ResultMessage findTrmmForSeven(Long[] regionId, String date) {
        ...
        if(regionId.length>0&&StringUtils.isNotBlank(date))
        {
            ...
            List<TrmmSevenVo>  trmmList = trmmMapper.findTrmmForSeven(map);
   ...
   */
    @Override
    public ResultMessage findTrmmForReport(String chinaName,Long regionId, String startDate, String endDate,Integer resolution ) {
        //检验参数，校验通过方法才可继续
        ResultMessage result = paramCheck(regionId,startDate,endDate,resolution);
        List<TtnVo> list2 =new ArrayList<TtnVo>(); //最大降雨区域
        List<TtnVo> list3 =new ArrayList<TtnVo>(); //各区域值总和
        List<TtnVo> list4 =new ArrayList<TtnVo>(); //各区中最大降雨值
        ...
        if(result.isFlag()) {
            ...
            list3 = trmmMapper.findTrmmById(param);
            if (list3.size() > 0) {
                list2 = trmmMapper.findTrmmMaxByDate(param);
                list4 = trmmMapper.findMaxTrmmById(param);
                map = findTrmmForTwoById(param);
                paramData.setStartDate(st.plusDays(-30));
                paramData.setEndDate(et.plusDays(-30));
                double drn = trmmMapper.findSumTrmmById(paramData); //上一期降水值和
                ...
                //构造降温报告对象
                ReportVo rp = makeTrmmReportObject(chinaName, startDate, endDate, addNum1, addNumText1, addNum2, addNumText2, addNum3, addNumText3, drn, d1, d2, d3, d4, list2, list3, list4, "trmm");
                String reStr;
                if (list3.size() == 1){
                     reStr = generateReportUtil.generateReportByTemplate(rp, "noBeyondTrmmReporter.html", "trmm");
                }else {
                     reStr = generateReportUtil.generateReportByTemplate(rp, "trmmReporter.html", "trmm");
                }

                if (StringUtils.isNotBlank(reStr)) {
                    result = ResultMessage.success("trmm", "生成降雨报告成功", reStr);
                } else {
                    result = ResultMessage.fail("trmm", "生成降雨报告失败");
                }
            }
            else{
                result = ResultMessage.fail("trmm", "降雨数据为空");
            }
        }
        return result;
    }
    ...
    /*
  * @description:根据区域构造某一时间段内降雨的近二年和10年均值数据查询服务
  * @param param 降雨数据集参数对象
  * @return
  * @Version <1> 2018-05-02 cxw: Created.
  */
    public Map<String,Object> findTrmmForTwoById(TtnEntity param) {
        int avgYearNum = 10; //10年平均
        int showYearNum = 2; //显示最近二年数据
        //用于保存二年及十年均值
        Map<String,Object> allMap = new HashMap<String,Object>();
        //临时保存十年均值
        List<TtnVo> avgList = new ArrayList<TtnVo>();
        ...
        for(int i=0;i<avgYearNum;i++){
            //获取每一年的数据，并将年做为键，保存三年的数据
            List<TtnVo> dataList = trmmMapper.findTrmmForMonById(param);
            int year = param.getStartDate().getYear();
            String key = year + "";
            if(i<showYearNum){
                allMap.put(key,dataList);
     ...
     */
    public ReportVo makeTrmmReportObject(String chinaName,String startDate,String endDate,double addNum1,String addNumText1,double addNum2,String addNumText2,
        ...
        String levelStr = "";
        if(mxDate!=null&&maxArea!=null&&maxData!=null)
        {
            if(maxData==0) levelStr = "无雨";
            if(maxData>=0.1&&maxData<=9.9) levelStr = "为小雨";
            if(maxData>=5&&maxData<=16.9) levelStr = "为小到中雨";
            if(maxData>=17&&maxData<=37.9) levelStr = "为中到大雨";
            if(maxData>=25&&maxData<=49.9) levelStr = "为大雨";
            if(maxData>=38&&maxData<=74.9) levelStr = "为大到暴雨";
            if(maxData>=50&&maxData<=99.9) levelStr = "为暴雨";
            if(maxData>=75&maxData<=174.9) levelStr = "为暴雨到大暴雨";
            if(maxData>=100&&maxData<=250) levelStr = "为大暴雨";
            if(maxData>=175&&maxData<=300) levelStr = "为大暴雨到特大暴雨";
            if(maxData>=250) levelStr = "为特大暴雨，可能带来严重水患";
        }
        rp.setLevelStr(levelStr);
        return  rp;
    }
     ...
     */
    @Override
    public ResultMessage findTrmmTimes(Long regionId, String startDate, String endDate, Integer accuracyId) {
        // 校验参数
        ...
        if (result.isFlag()) {
            ...
            dataTimes = trmmMapper.findTrmmTimes(param);
            if (dataTimes.size() > 0) {
                result.setData(dataTimes);
            } else {
                result = ResultMessage.fail("无降雨数据");
            }
        }
    ...
    public ResultMessage findAllTrmm(Long regionId, String startDate, String endDate) {
        ...
        if (result.isFlag()) {
            ...
            list = trmmMapper.findAllTrmm(param);
            result.setData(list);
        }
        return result;
    }
