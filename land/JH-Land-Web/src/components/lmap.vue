...
export default {
  ...
  data() {
    return {
      ...
      // 底图地址：天地图影像
      baseMapLayerUrl:
        "http://t{s}.tianditu.gov.cn/DataServer?T=img_w&x={x}&y={y}&l={z}&tk=d7b4a2504ed9ff285644059f12613233",
      // 行政区划边界地址
      lyrRegionLayerUrl: this.baseApiPath + "/jh-map/nolog/layer/ows",
      // 获取地块图层名称地址
      getLandLayerNameUrl:
        this.baseApiPath + "/jh-land/nolog/rankAreaTask/findLayerName",
      // 地块TILE-WMS服务地址
      lyrTileWMSUrl: this.baseApiPath + "/jh-map/nolog/layer/wms",
      // 根据经纬度查询地块地址
      getLandByLonlat:
        this.baseApiPath + "/jh-land/nolog/land/findLandInfoByLonLat",
      // 根据ID查询地块地址
  ...
  methods: {
    ...
    bindTriggers() {
      ...
      JHGEvent.on("Map/ClickLand", rankId => {
        ...
        let landTableName =
          "init_rank_" + that.dateYear + "_" + that.currentCityRegionId;
        that.$axios
          .post(that.getLandById, {
    ...
    showLandPopup(data, toCenter) {
      ...
      content +=
        '<div style="width:180px;font-weight: bold;font-size: 14px;">' +
        rankName +
        "</div>";
      content +=
        '<div style="margin-left:10px;"><a rankId="' +
        rankId +
        '"href="javascript:void(0);">详情</a></div>';
      content += "</div>";
      // 作物信息
      ...
      content +=
        '<div style="font-size:16px;margin-top:5px;">' + cropName + "</div>";
      content += "</div>";
      // 面积
      content += '<div style="width:50%;">';
      content += '<div style="font-size:12px;color: #707070;">面积（亩）</div>';
      content +=
        '<div style="font-size:16px;margin-top:5px;">' + area + "</div>";
      content += "</div>";
      content += "</div>";
      // 创建新的POPUP
      let lon = data.lon;
    ...
    renderRegionLabelList(features) {
      ...
      features.forEach(feature => {
        ...
        html +=
          '<div style="font-size:14px;padding:3px 3px;">' +
          chinaName +
          "</div>";
        html += "</div>";
    ...
    getFilterParam(regionId, cropId) {
      ...
      if (regionId) {
        filter +=
          "<PropertyIsEqualTo><PropertyName>parent_id</PropertyName><Literal>" +
          regionId +
          "</Literal></PropertyIsEqualTo>";
      }
      if (cropId) {
        filter +=
          "<PropertyIsEqualTo><PropertyName>crop_id</PropertyName><Literal>" +
          cropId +
          "</Literal></PropertyIsEqualTo>";
      }
      filter += "</And></Filter>";
